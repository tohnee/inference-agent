...
- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/kernel-benchmark/`
  - 统一 benchmark 入口，支持：
    - CUDA/CUTLASS `.cu`（`extern "C" void solve(...)`）
    - Triton `.py`（`setup(...) + run_kernel(...)`）
  - 可选 correctness 对比 reference

- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/ncu-rep-analyze/`
  - 生成或分析 `.ncu-rep`
  - 解释 NCU 指标并映射到可执行优化方向

- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/operator-optimize-loop/`
  - 统一编排入口：
    - correctness -> benchmark -> targeted/full NCU -> proposal -> 下一版算子 -> 再评测
  - 支持策略记忆（positive / negative / rejected）

- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/reference/`
  - 按后端维护知识库：
    - `reference/cuda/`
    - `reference/cutlass/`
    - `reference/triton/`
...
需要完整迭代优化时优先使用：
- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/operator-optimize-loop/SKILL.md`

只做 correctness + benchmark：
- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/kernel-benchmark/SKILL.md`

只做 NCU 报告生成与分析：
- `cuda-kernel-opt-skill/skills/cuda-optimized-skill/ncu-rep-analyze/SKILL.md`
...
- 全局：`cuda-kernel-opt-skill/skills/cuda-optimized-skill/operator-optimize-loop/strategy-memory/global_strategy_memory.json`
...
```bash
python cuda-kernel-opt-skill/skills/cuda-optimized-skill/kernel-benchmark/scripts/benchmark.py path/to/kernel.cu \
...
```bash
python cuda-kernel-opt-skill/skills/cuda-optimized-skill/operator-optimize-loop/scripts/optimize_loop.py path/to/kernel.cu \
    --backend=cuda --ref=path/to/reference.py --M=4096 --N=4096 --K=4096 \
...
```bash
python cuda-kernel-opt-skill/skills/cuda-optimized-skill/operator-optimize-loop/scripts/optimize_loop.py path/to/cutlass_kernel.cu \
...
```bash
python cuda-kernel-opt-skill/skills/cuda-optimized-skill/operator-optimize-loop/scripts/optimize_loop.py path/to/triton_kernel.py \
